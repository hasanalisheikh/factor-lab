...
This section reconciles the PRD with the current repository state as of the stabilization audit. Use
`Implemented` for shipped behavior, `Partially implemented` where the core exists but scope differs
from the PRD, `Not implemented` for missing behavior, and `Out of date` where the PRD no longer
matches the product direction.

| Criterion                                 | Status                | Evidence                                                                                                                                                                                                                          | Notes                                                                                                                                                            |
| ----------------------------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Auth, sign up/sign in/sign out, guest use | Implemented           | `app/login/page.tsx`, `components/auth/login-form.tsx`, `app/api/auth/guest/route.ts`, `app/actions/auth.ts`, `proxy.ts`                                                                                                          | Guest users are real Supabase users; guest upgrade and sign-out flows exist.                                                                                     |
| User and guest isolation                  | Implemented           | `supabase/migrations/20260321_isolation_rls.sql`, `lib/supabase/isolation-rls.test.ts`, `lib/supabase/queries/runs.ts`, `app/actions/runs/create-run.ts`                                                                          | Owner-scoped RLS and app-level user filters protect runs, jobs, reports, positions, and notifications.                                                           |
| Run creation, queue, and progress         | Implemented           | `components/run-form/*`, `app/actions/runs/create-run.ts`, `components/run-detail/job-status-panel.tsx`, `components/run-detail/run-status-poller.tsx`, `services/engine/factorlab_engine/repositories/jobs.py`                   | Runs preflight data coverage, create jobs, wake the worker, and poll progress with backoff.                                                                      |
| Results tabs                              | Implemented           | `app/(dashboard)/runs/[id]/page.tsx`, `components/run-detail/overview-tab.tsx`, `components/run-detail/holdings-tab.tsx`, `components/run-detail/trades-tab.tsx`, `components/run-detail/ml-insights-tab.tsx`                     | Overview, Holdings, Trades, and ML Insights are present, with ML insights shown for ML runs.                                                                     |
| Multi-benchmark support                   | Implemented           | `lib/benchmark.ts`, `app/actions/runs/schema.ts`, `components/run-form/run-form-fields.tsx`, `services/engine/factorlab_engine/worker/execution.py`, `lib/report-builder.ts`                                                      | The selected benchmark is stored and used in the UI, worker, Compare, and reports.                                                                               |
| Equity charts show the full run           | Implemented           | `lib/equity-curve.ts`, `components/equity-chart.tsx`, `components/dashboard-overview.tsx`                                                                                                                                         | Long runs default to all available history while downsampling preserves the full span.                                                                           |
| Benchmark overlap warning                 | Implemented           | `lib/benchmark.ts`, `lib/supabase/queries/runs.ts`, `components/benchmark-overlap-warning.tsx`, `app/(dashboard)/runs/[id]/page.tsx`, `components/dashboard-overview.tsx`, `lib/report-builder.ts`                                | Confirmed overlap warnings appear in run detail, dashboard, and reports.                                                                                         |
| Dashboard                                 | Implemented           | `app/(dashboard)/dashboard/page.tsx`, `components/dashboard-overview.tsx`, `components/recent-runs.tsx`, `components/runs-table.tsx`                                                                                              | Shows featured run KPIs, equity preview, recent runs, and overlap context.                                                                                       |
| Compare                                   | Implemented           | `app/(dashboard)/compare/page.tsx`, `components/compare/compare-workbench.tsx`, `lib/supabase/queries.compare.test.ts`                                                                                                            | Supports completed-run comparison with KPI, equity, and drawdown views.                                                                                          |
| Strategies and guide                      | Implemented           | `app/(dashboard)/strategies/page.tsx`, `app/(dashboard)/guide/page.tsx`, `docs/strategies.md`, `docs/user-guide.md`                                                                                                               | Methodology, workflow, warnings, disclaimers, and ML details are documented.                                                                                     |
| Data Health and ingestion                 | Partially implemented | `app/(dashboard)/data/page.tsx`, `components/data/benchmark-coverage-card.tsx`, `app/api/data/ingest-benchmark/route.ts`, `app/actions/runs/preflight-issues.ts`, `lib/data-health.ts`, `lib/coverage-check.ts`                   | Health metrics and repair jobs exist, but manual repair controls are diagnostics-gated and the default product path relies mainly on preflight/scheduled repair. |
| Reports / tearsheets                      | Implemented           | `app/actions/reports.ts`, `components/run-detail/generate-report-button.tsx`, `lib/report-builder.ts`, `lib/storage.ts`, `supabase/migrations/20260221070000_phase7_reporting.sql`                                                | Generates UTF-8 HTML, uploads to Supabase Storage, stores report metadata, and exposes download links.                                                           |
| Settings                                  | Partially implemented | `app/(dashboard)/settings/page.tsx`, `components/settings-form.tsx`, `app/actions/settings.ts`, `app/actions/account.ts`, `supabase/migrations/20260303_settings_extended.sql`                                                    | Backtest and account settings exist; sign out lives in the topbar/profile menu rather than the Settings account tab.                                             |
| Notifications, search, and profile        | Partially implemented | `components/layout/topbar.tsx`, `components/layout/topbar-search.tsx`, `components/layout/topbar-notifications.tsx`, `components/layout/topbar-user-menu.tsx`, `components/runs-search-bar.tsx`, `app/(dashboard)/jobs/page.tsx`  | Notifications and profile actions exist; topbar search routes to run search but is not a full global jobs/runs search.                                           |
| Security and service-role boundaries      | Implemented           | `lib/supabase/admin.ts`, `app/actions/reports.ts`, `app/api/data/ingest-benchmark/route.ts`, `services/engine/factorlab_engine/worker/http_server.py`, `supabase/migrations/20260321_isolation_rls.sql`                           | Service-role paths are server-only and ownership-gated; fail-closed rate limiting remains future hardening before higher-risk ingest UX.                         |
| Performance                               | Implemented           | `supabase/migrations/20260305020000_performance_indexes.sql`, `supabase/migrations/20260618090000_task6_performance_indexes.sql`, `lib/coverage-check.ts`, `components/active-runs-poller.tsx`, `lib/supabase/queries/runs.ts`    | User-scoped indexes, bounded list queries, batched coverage checks, and polling backoff are in place.                                                            |
| ML Top N clamping and diagnostics         | Partially implemented | `services/engine/factorlab_engine/worker/execution.py`, `services/engine/factorlab_engine/ml.py`, `services/engine/tests/test_ml.py`, `services/engine/tests/test_worker_ml_dispatch.py`, `lib/__tests__/strategy-warmup.test.ts` | Worker validation and tests exist, but this audit did not complete an end-to-end ML smoke run.                                                                   |
