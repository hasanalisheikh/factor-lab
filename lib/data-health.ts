...
const BENCHMARK_MISSING_GOOD = 0.01
const BENCHMARK_MISSING_DEGRADED = 0.02
...
export function summarizeInceptionAwareCoverage(params: {
  ...
  const effectiveGlobalStart =
    globalStart ??
    ranges.reduce((min, range) => (!min || range.firstDate < min ? range.firstDate : min), null as string | null) ??
    ""
  ...
  const rows = ranges
    ...
    .map((range) => {
      ...
      return {
        ...
        trueMissingDays,
        preInceptionDays,
        coveragePercent: expectedDays > 0 ? Math.min((range.actualDays / expectedDays) * 100, 100) : 100,
      }
    })
...
export function assessDataHealth(metrics: DataHealthMetrics): DataHealthAssessment {
  ...
  if (isGood) {
    return {
      status: "GOOD",
      reason: makeReason(`all monitored metrics, including benchmark ${metrics.benchmarkTicker}, are within good thresholds`),
    }
  }
  ...
  return {
    ...
    reason:
      ...
      makeReason("one or more monitored metrics are outside the good range but below degraded thresholds"),
  }
}
