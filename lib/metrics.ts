...
export function inferAnnualizationFactor(dates: string[]): number {
  ...
  const totalMs = new Date(dates[dates.length - 1]).getTime() - new Date(dates[0]).getTime();
...
function seriesMetrics(dates: string[], values: number[], annFactor: number): SeriesMetrics {
...
export function computeMetrics(
  ...
  const startVal =
    portfolioValues.length > 0 && Number.isFinite(portfolioValues[0]) && portfolioValues[0] !== 0
      ? portfolioValues[0]
      : 1;
  const equity = portfolioValues.length >= 1 ? portfolioValues.map((v) => v / startVal) : [];

  const drawdown = portfolioValues.length >= 2 ? drawdownSeries(portfolioValues) : [];
