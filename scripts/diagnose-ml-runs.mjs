...
async function fetchBundle(runId) {
  const [metrics, positions, equity] = await Promise.all([
    ...
    supabase.from("positions").select("date,symbol,weight").eq("run_id", runId).order("date", { ascending: true }),
    supabase.from("equity_curve").select("date,portfolio,benchmark").eq("run_id", runId).order("date", { ascending: true }),
  ])
...
async function main() {
  ...
  console.log("Ridge run:", ridge.id, ridge.strategy_id, ridgeData.metrics, ridge.run_metadata ?? {})
  console.log("LightGBM run:", lgbm.id, lgbm.strategy_id, lgbmData.metrics, lgbm.run_metadata ?? {})
