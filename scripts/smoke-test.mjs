...
const flagStrategies = args.find((a) => a.startsWith("--strategies="))
  ?? args[args.indexOf("--strategies") + 1]
const flagTimeout = args.find((a) => a.startsWith("--timeout="))
...
const STRATEGIES_TO_TEST = flagStrategies
  ? String(flagStrategies).replace("--strategies=", "").split(",").map((s) => s.trim())
  : ALL_STRATEGIES

const POLL_TIMEOUT_MS = (parseInt(
  String(flagTimeout ?? "").replace("--timeout=", ""),
  10
) || 300) * 1000  // default 300 seconds (5 min)
...
async function assertOutputs(runId, label) {
  ...
  const [metricsRes, equityRes, positionsRes] = await Promise.all([
    supabase.from("run_metrics").select("cagr,sharpe,max_drawdown").eq("run_id", runId).maybeSingle(),
    supabase.from("equity_curve").select("date,portfolio,benchmark", { count: "exact" }).eq("run_id", runId).limit(1),
    supabase.from("positions").select("date,symbol,weight", { count: "exact" }).eq("run_id", runId).limit(1),
  ])
  ...
  log(label, `outputs OK (metrics ✓, equity_curve count=${equityRes.count} ✓, positions count=${positionsRes.count} ✓)`)
  return true
}
